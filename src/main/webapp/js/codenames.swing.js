...
function updateInfo(game, team) {
    ...
    if (game.over) {
        ...
        turnInfo.addClass("game-over");
        turnInfo.addClass("anim-border");
        setTimeout(function(){turnInfo.removeClass("anim-border");}, 1000);
    } else {
        turnMessage.text('Turno del ' + game.teams[game.turnId].name);
        turnMessage.addClass('text-' + game.teams[game.turnId].color);
        turnInfo.removeClass("game-over");
    }
...
function updatePlayerlist(players, teams) {
    for (let i in teams) {
        ...
        if (teamTable.length === 0) {
            ...
            let btnTile = $("<img style='cursor:pointer;max-height:75px' class='img-fluid' src='images/team-" + teams[i].name + "-logo.png' title='Unirse a " + teams[i].name + "' alt='team-" + teams[i].name + "-logo'/>").on("click", function () {
